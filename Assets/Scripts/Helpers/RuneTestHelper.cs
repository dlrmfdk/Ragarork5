...
public class RuneTestHelper : MonoBehaviour
{
    ...
    public BaseRuneEffectSO effectToTestOnAlpha2; // 테스트할 효과 SO (인스펙터에서 할당)
    ...
    void Update()
    {
        ...
        // 알파 숫자 2 키를 눌렀을 때 (예시로 다른 효과를 테스트할 수 있습니다)
        if (Input.GetKeyDown(KeyCode.Alpha2))
        {
            TestSpecificEffect2(effectToTestOnAlpha2, "알파 2");
        }
        // 알파 숫자 3 키를 눌렀을 때 (턴 넘기기)
        if (Input.GetKeyDown(KeyCode.Alpha3))
        {
            ...
            {
                Debug.LogWarning("[Rune Test Key - 알파 3] TurnManager 인스턴스를 찾을 수 없습니다. 턴을 넘길 수 없습니다.");
            }
    ...
    private void TestSpecificEffect2(BaseRuneEffectSO effectSO, string keyName)
    {
        if (effectSO == null)
        {
            Debug.LogWarning($"[Rune Test Key - {keyName}] 테스트할 효과(EffectSO)가 할당되지 않았습니다. RuneTestHelper 인스펙터에서 설정해주세요.");
